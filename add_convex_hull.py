...
def add_convex_hull_column(df):
    ...
    df['convex_hull'] = df['Res_Num'].map(res_to_ch).fillna(k).astype(int)
    return df
...
def main(
    ...
    for i,value_dict in tqdm(dataset_dict.items()):
        ...
        df_pdb = format_pdb(f"/home/gathenes/all_structures/imgt/{pdb}.pdb").query("Atom_Name=='CA' and Chain.isin(@chains)").astype({"x":float,"y":float,"z":float})

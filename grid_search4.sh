                        ...
                        for d2 in "${dim2s[@]}" ; do
                            for d3 in "${dim3s[@]}" ; do
                                for alpha in "${alphas[@]}" ; do
                                    echo "learning rate" $lr "batchnorm" $bn "dropout" $dr "maskprob" $mk "batchsize" $bs "dim1" $d1 "dim2" $d2 "dim3" $d3 "alpha" $alpha
                                    result_folder=$folder/lr-${lr}_bn-${bn}_dr-${dr}_mk-${mk}_bs-${bs}_dim1-${d1}_dim2-${d2}_dim3-${d3}_alpha-${alpha}
                                    mkdir -p $result_folder
                                    python train.py $train $val -n 500 --lr $lr $bn --dropout $dr --mask-prob $mk -bs $bs --dim1 $d1 --dim2 $d2 --dim3 $d3 -r $result_folder --alpha $alpha > $result_folder/train_logs.txt

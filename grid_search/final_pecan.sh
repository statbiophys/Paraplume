...
folder=/home/athenes/benchmark2/pecan/250104/
lrs=("0.00001")
batch_norms=("")
                                    ...
                                    for ds in "${distances[@]}" ; do
                                        for seed in "${seeds[@]}" ; do
                                            for emb in "${embeddings[@]}" ; do
                                                ...
                                                python train.py $train $val -n 500 --lr $lr --dropouts $dr --mask-prob $mk -bs $bs --dims $d1 -r $result_folder --alphas $alpha --l2-pen $pen --pos-weight $weight $cv $ds --seed $seed --emb-models $emb --patience 10 --override --gpu 0 > $result_folder/train_logs.txt

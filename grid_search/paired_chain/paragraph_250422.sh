...
train=/home/athenes/benchmark3/paragraph/paired_chain/train_set
val=/home/athenes/benchmark3/paragraph/paired_chain/val_set
test=/home/athenes/benchmark3/paragraph/paired_chain/test_set
                            ...
                            for weight in "${weights[@]}" ; do
                                for seed in "${seeds[@]}" ; do
                                    for emb in "${embeddings[@]}" ; do
                                        ...
                                        python paraplume/paired_chain/train.py $train $val --gpu 1 -n 500 --lr $lr --dropouts $dr --mask-prob $mk -bs $bs --dims $d1 -r $result_folder --alphas $alpha --l2-pen $pen --pos-weight $weight --seed $seed --emb-models $emb --patience 20 > $result_folder/train_logs.txt
                                        python paraplume/paired_chain/predict.py $result_folder/checkpoint.pt $test $test_csv --pdb-folder-path $pdb_folder
                                    done
                                done
                            done
                        done

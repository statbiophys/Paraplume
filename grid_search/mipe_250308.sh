...
folder=/home/athenes/benchmark2/mipe/250308/
lrs=("0.00005")
                                ...
                                for seed in "${seeds[@]}" ; do
                                    for emb in "${embeddings[@]}" ; do
                                        for cv in "${cvs[@]}" ; do
                                            ...
                                            python train.py ${train}_${cv} ${val}_${cv} --gpu 1 -n 500 --lr $lr --dropouts $dr --mask-prob $mk -bs $bs --dims $d1 -r $result_folder --alphas $alpha --l2-pen $pen --pos-weight $weight --seed $seed --emb-models $emb --patience 20 > $result_folder/train_logs.txt

...
from models import MLP, EarlyStopping, MLP_final
...
def main(
    ...
    model_list_paths:str=typer.Argument(
        ...,
        help="List of model paths, separated by commas."
    ),
    ...
    args_dict = {
        ...
        "override":override,
        "model_list_paths":model_list_paths,
        "alpha":alpha,
    }
    ...
    model_list_paths = model_list_paths.split(",")
    model_list=[]
    for model_path in model_list_paths:
        model_path=train_folder_path.parents[0] / Path(model_path)

        with open(model_path / Path("summary_dict.json")) as f :
            summary = json.load(f)
        dim1, dim2, dim3, batch_norm =int(summary["dim1"]), int(summary["dim2"]), int(summary["dim3"]), summary["batch_norm"]
        mod=MLP(dim1=dim1, dim2=dim2, dim3=dim3,batch_norm=batch_norm)
    ...
    model = MLP_final(dim=len(model_list))
    criterion = nn.BCELoss()

...
from paraplume.utils_single import get_embedding_single, get_outputs_single
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for i, (
            ...
            *other_labels,
            ...
            embedding, main_labels, chain_length_tensor = (
                embedding.to(device),
                main_labels.to(device),
                chain_length_tensor.to(device)
            )
            ...
            for i in range(chain_length_tensor.shape[-1]):
                ...
                lab_main = main_labels[i][: chain_length_i]
                main_labels_list.append(lab_main)
                for l, each in enumerate(other_labels):
                    other_label = each[i][: chain_length_i]
        ...
        with torch.no_grad():
            ...
            model.eval()
            for embedding, labels, chain_length_tensor in val_loader:
                embedding, labels = embedding.to(device), labels.to(device)
                labels, output = get_outputs_single(
                    embedding=embedding,
                    labels=labels,
                    chain_length_tensor=chain_length_tensor,
                    model=model,
...
def main(
    ...
    chain:str=typer.Option(
        "chain",
        "--chain",
        help="Chain to use for single chain model. Defaults to chain.",
    )
) -> None:

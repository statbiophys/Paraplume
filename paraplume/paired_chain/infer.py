...
from paraplume.utils_paired import process_embedding_paired
...
def predict_from_df(
    ...
    llm_to_embedding_dict = get_llm_to_embedding_dict(sequences_heavy,
                                                    sequences_light,
    ...
    embeddings_paratope=[]
    for i, (seq_heavy, seq_light) in enumerate(zip(sequences_heavy,sequences_light)):
        heavy, light = len(seq_heavy), len(seq_light)
        emb_llm_list_i=[]
        for llm, emb_llm in llm_to_embedding_dict.items():
            emb_processed_i_llm = process_embedding_paired(
                llm=llm,
                emb=emb_llm[i],
                heavy=heavy,
                light=light,
...
def predict_from_sequence(
    ...
    if llm_models == "all":
        llm_models = "ablang2,igbert,igT5,esm,antiberty,prot-t5"
    llm_list = llm_models.split(",")
    llm_to_embedding_dict = get_llm_to_embedding_dict([sequence_heavy],
                                                    [sequence_light],
                                                    emb_proc_size=1,
                                                    llm_list=llm_list,
                                                    heavy_chain=heavy_chain,
    ...
    for llm,emb in llm_to_embedding_dict.items():
        emb_sequence=emb[0]
        emb_processed = process_embedding_paired(
            llm=llm,
            emb=emb_sequence,
            heavy=heavy,
            light=light,
    ...
    print("===== Heavy Chain =====")
    for each in zip(sequence_heavy,output[:heavy]):
        print(f"{each[0]}  --> {np.round(float(each[1]),3)}")
    print("===== Light Chain =====")
    for each in zip(sequence_light,output[heavy:heavy+light]):
        print(f"{each[0]}  --> {np.round(float(each[1]),3)}")
    return output

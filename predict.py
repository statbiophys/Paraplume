...
from utils import get_embedding, get_logger, read_pdb_to_dataframe
...
def test(
    ...
    with torch.no_grad():
        for i, (
            ...
            emb = get_embedding(embedding=embedding, embedding_models=embedding_model_list, heavy=heavy, light=light)
            output=model(emb)
    ...
    return total_dataframe, mean_ap, mean_roc, mean_f1, mean_mcc, flattened_ap, flattened_auc,flattened_f1, flattened_mcc
...
def main(
    ...
    save:bool = typer.Option(
        False,
        "--save",
        help="Whether to save pdbs or not."
    ),
